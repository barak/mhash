 ...
 */
void
haval_string(char *string, unsigned char fingerprint[HAVAL_FPTLEN >> 3])
{
	haval_state state;
 ...
 */
int
haval_file(char *file_name, unsigned char fingerprint[HAVAL_FPTLEN >> 3])
{
	FILE *file;
	haval_state state;
	int len;
...
haval_stdin(void)
{
	haval_state state;
	int i, len;
	unsigned char buffer[32], fingerprint[HAVAL_FPTLEN >> 3];
 ...
 */
void
haval_start(haval_state * state)
{
	state->count[0] = state->count[1] = 0;	/*
											   clear count 
											 */
	state->fingerprint[0] = 0x243F6A88;		/*
											   initial fingerprint 
											 */
 ...
 */
void
haval_end(haval_state * state, unsigned char final_fpt[HAVAL_FPTLEN >> 3])
{
	 ...
	 */
	uint2ch(state->fingerprint, final_fpt, HAVAL_FPTLEN >> 5);
...
{
	register haval_word t0 = state->fingerprint[0],		/*
														   make use of 
														 */
	 t1 = state->fingerprint[1],	/*
									   internal registers 
									 */
	 t2 = state->fingerprint[2], t3 = state->fingerprint[3], t4 = state->fingerprint[4],
	 t5 = state->fingerprint[5], t6 = state->fingerprint[6], t7 = state->fingerprint[7],
	*w = state->block;
 ...
 */
static void
haval_tailor(haval_state * state)
{
	haval_word temp;
